...
The `@udl/plugin-source-contentful` plugin sources all content types, entries, and assets from a Contentful space. It uses Contentful's Sync API for efficient incremental updates.
...
Linked entries and assets are stored as references:
...
For the complete API reference, see the [package README](https://github.com/anthropics/universal-data-layer/tree/main/packages/plugin-source-contentful).
